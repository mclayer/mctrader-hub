...
- **Phase 4 와 race 회피**:
  ...
  3. ADR amendment 충돌 회피 — 본 Story 는 ADR-009 amendment 0건. ADR-015 cross-ref optional. Phase 4 = ADR-016 단독 amendment. Overlap 0.

- **Phase 4 surface review reconciliation** (parallel session memory `project_dockerization_phase4.md` 의 4 surface item 검토):
  - **Surface 1 (ADR-014 single-process invariant)** — engine paper_runner 가 asyncio.Runner.run() 단일 process + HealthServer daemon thread 패턴 → 이미 align.
  - **Surface 2 (HealthServer pattern)** — Pilot stdlib http.server 패턴 채택 (FastAPI 의존 추가 회피, 단순성 우선). web 의 `/admin/health` 차용 권장은 reject — Pilot precedent 일관성 우선.
  - **Surface 3 (cross-stack volume name)** — Pilot compose.yml line 38 의 explicit `name: mctrader_data` 의존성을 본 Story compose 의 external volume declaration 에 명시 (`name: mctrader_data` 추가). standalone fallback (volume 부재 시 graceful) = F8 carry-over.
  - **Surface 4 (ADR-002 D6 paper ledger forward-only)** — Phase 3 scope 외, F9 carry-over (별도 ADR amendment).
...
| F8 | engine standalone fallback (mctrader_data volume 부재 시 paper/backtest 실패 graceful 처리) — Phase 4 surface 3 carry-over | 별도 Story 후보 (현재는 volume 부재 = scan_candles empty result, 사용자 자명 detect) |
| F9 | ADR-002 D6 paper ledger forward-only invariant 의 Docker volume 박제 — Phase 4 surface 4 carry-over | 별도 ADR amendment (ADR-002 D6 또는 신규 ADR), engine paper ledger event_store / paper_partition / equity 의 forward-only 정의 — `mctrader_engine_runs` volume 보존 의무 강화 |
