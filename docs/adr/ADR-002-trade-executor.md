...
**MCT-104 Phase 1 PR 이 H5 first implementation seal — 단 produce-only seal**:

- **Source partition**: ADR-009 §D13 (`exchange_metadata.v1`) — mctrader-data collector daemon 이 daily cadence 로 Bithumb public REST 호출 후 적재. **단 Phase 2 wiretap (P2-F-003) 검증 결과 — Bithumb public REST 가 tick_size / min_order_qty / fee_maker / fee_taker / min_order_notional_krw 미노출**. ADR-009 §D13.10 (Public-fillable subset) 절 박제대로 Phase 2 = NULL, Live Epic 진입 시 hand-coded lookup + private endpoint 로 채움.
- **Capability 매핑** (§D13.6 박제):
  - `fee_maker` / `fee_taker` → `Capabilities.fee` (maker/taker 분리 필수, ADR-002 H5 의 "fee" 단일 field 가 maker/taker 분리로 명시화 amend) — **Phase 2 = NULL**
  - `tick_size` → `Capabilities.tick` — **Phase 2 = NULL**
  - `min_order_qty` → `Capabilities.min_order_size` — **Phase 2 = NULL**
  - `min_order_notional_krw` → `Capabilities.min_order_notional` — **Phase 2 = NULL**
  - `asset_status` → ADR-002 H9 data freshness gate 의 입력 (asset halt = order reject) — **Phase 2 fillable**
- **Seal 의미 단계화** (P2-F-006 wiretap):
  - **MCT-104 (Phase 2) = produce-only seal** — partition shape + symbol/asset_status 적재 박제. 5 quantization column NULL. **dormant 상태 부분 종결** (capability shape + 채움 mechanism 의 절반).
  - **Live Epic 진입 Story (별도 발의 의무) = produce + consume seal** — 5 quantization column NULL→fill (hand-coded lookup + private endpoint) + mctrader-engine 측 `ExchangeCapabilities` consumer 구현 (backtest/paper/live 모두). dormant 완전 종결.
  - **본 Story (MCT-104) §11 carry-over**: H5 consumer follow-up Story 발의 의무 (Live Epic 의존성 아니라 본 Story 후속 Phase).
- **Live executor consumer 책임**: mctrader-engine 측 `ExchangeCapabilities` consumer 구현 = Live executor Epic (별도 Story) 책임. MCT-104 = produce only — `exchange_metadata.v1` partition 적재 + symbol/asset_status 채움까지 책임.
...
- **NULL → fill transition path**: 동일 partition (§D13.2 layout) 에 NULL column 채운 새 row append (forward-only). data_hash 변경 → reader 가 자연스럽게 새 capability 채택. Backward-compat 0 breakage. (§D13.10 박제)

본 amendment 로 ADR-002 H5 의 dormant 상태가 **부분 종결** (produce-only seal) 되며, ADR-009 §D13 이 H5 의 canonical source 로 박제된다. ADR-009 §D13.6 의 "ADR-002 H5 매핑" 절 + §D13.10 (Public-fillable subset) 이 SSOT 으로 cross-reference. **완전 종결 = consumer Story 발의 후**.
