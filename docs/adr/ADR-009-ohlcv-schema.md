...
**MCT-167 amendment 박제 (2026-05-14, EPIC-tier-promotion-single-source, ADR-029 publish)** — forward-only invariant 의 enforcement layer 격상.

ADR-009 §D12.2 원안 = forward-only invariant 의 enforcement layer = **local file system** (named volume `mctrader_data` 의 영속 보장 + collector lifecycle hard constraint). 본 amendment 박제 후 enforcement layer = **NAS object SoT (versioning 기반)** 격상.

- **NAS = Single Source of Truth (all tiers)** (ADR-029 D1+D10 + ADR-027 §D9 amendment 정합) — L1 (5min Parquet) + L2 (1h Parquet) + L3 (1day Parquet) 모두 NAS authoritative.
- **local = ephemeral cache only** — engine + compactor 호스트 측 local file = read latency optimization (LRU/TTL cache), write authority 0.
- **tier promotion 후 local delete = forward-only 위반 0** (ADR-029 D3=C) — L1 → L2 promote 시 L1 local 즉시 삭제, L2 → L3 promote 시 L2 local 즉시 삭제. delete trigger = NAS HEAD verify (version/etag exact match + sha256 verify) 완료 직후. ambiguity 차단 invariant (ADR-029 D10) = "tier promotion 완료 후 동일 logical entity 가 NAS + local 양쪽 동시 존재 시 violation".
- **NAS bucket versioning prerequisite** (MCT-161 LAND, 2026-05-14) — `mctrader-market` bucket versioning Enabled + Object Lock governance 30d + NoncurrentVersionExpiration 30d. 의도치 않은 delete/overwrite 의 복원 source = bucket version history (DR runbook `docs/runbooks/nas-bucket-disaster-recovery.md` 박제).
- **WAL local 유지 (ADR-029 D4=B 사용자 directive)** — WAL sealed segment = local only. RPO=0 보장 chain = L1 ParquetWriter atomic 직후 NAS PUT 단독 의존 (ADR-017 §3 D3 amendment 박제).
- **forward-only enforcement 강화**: NAS bucket version history 가 의도치 않은 forward-only 위반 (overwrite / delete 실수) 의 복원 source — local file system 단독 enforcement 대비 강한 invariant.
