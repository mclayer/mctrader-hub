...
### §6.1 VERIFIED amendment box (MCT-193 sub-3 carrier — PR-2 LAND VERIFIED)

MCT-193 = §6 enforcement timing 실 carrier. PR-1 docs draft → **PR-2 alert/cron LAND 후 VERIFIED 확정 (2026-05-17)**:

**PR-2 LAND 사실** — `monitoring/prometheus-alerts.yml` 신규 group `evidence-quad-enforcement` (`QuadViolationADR029NoDualWrite` + `QuadViolationADR030NoCollectorTicks`, `absent(<counter>) or increase(<counter>[14d]) == 0` critical for 0m, Q2=A absent() trap 차단. ADR-031 미등록 caveat 주석 — Q1=C dead-in-data, 기존 `mctrader-docker-stack` 5 alert 보존) + `.github/workflows/quad-evidence-audit.yml` **repo 최초 cron** (`on.schedule('0 2 1 * *') + on.workflow_dispatch` Q5=B hybrid, Prometheus HTTP API query → `gh issue create`, `PROMETHEUS_URL` 부재 시 graceful skip — MCT-179 D17 패턴, Q3=A alertmanager 부재 정합). **§6 VERIFIED 2026-05-17**.

- Prometheus alert rule: `monitoring/prometheus-alerts.yml` 신규 group `evidence-quad-enforcement` — ADR-029 QuadViolationADR029NoDualWrite + ADR-030 QuadViolationADR030NoCollectorTicks (`absent(<counter>) or increase(<counter>[14d]) == 0` critical, Q2=A absent() trap 차단). ADR-031 = 미등록 (Q1=C dead-in-data caveat — publish_tick producer caller=0, engine MCT-186 cutover 후 별 Story enable, rolling gate 영구 fire 차단, MCT-179 §D8 가공 metric 8회째 차단)
...
**§4 traffic class SSOT drift caveat (R-1, MCT-179 cross-doc SSOT drift 9회째 mitigation)**: §4 (line 121-127) `trading-hot path (collector tick) = market-open hours rolling (KRX 09:00-15:30 KST 7.5h/day × 10 trading days ≈ 75h)` 는 **Prometheus PromQL 로 KRX 거래일/공휴일 calendar 표현 구조적 불가**. MCT-193 Q4=A = production-wired **14d calendar 단일화** 채택 (trading-hot collector 도 14d). §4 market-open rolling 정의 = **후속 과제 carry** (recording rule + 외부 trading-calendar gate 필요, 별 Story). 본 caveat = §4 ↔ §6 자기모순 차단 (scope_manifest mct_193_decisions.Q4 ↔ 본 §6.1 ↔ alert yaml comment 3-source 1:1 reconcile, PR-1 DesignReview gate, MCT-179 c8e4b8e 전수 reconcile 패턴 reapply).
...
- **§9.2 sub-3 (MCT-193) — VERIFIED 2026-05-17**: PR-1 docs LAND (hub#387 3d79e1e — §6.1
  amendment box draft + §4 R-1 SSOT drift caveat + scope_manifest mct_193_* 6 블록 +
  counters IN_PROGRESS) + PR-2 alert/cron LAND (`monitoring/prometheus-alerts.yml`
  `evidence-quad-enforcement` group ADR-029/030 2 alert + `.github/workflows/quad-evidence-audit.yml`
  repo 최초 cron). §6 enforcement timing VERIFIED 확정 + frontmatter Proposed → Accepted
  (Q6=A 구현 LAND = transition, false-Accepted 차단) + ADR-031 미등록 dead-in-data 정직
  박제 (Q1=C). **§9.2 sub-3 VERIFIED 2026-05-17**.
...
**Accepted** (2026-05-17, sub-3 MCT-193 LAND — alert rule + cron 운영 carrier VERIFIED)
...
2026-05-17 — §6 enforcement timing 실 carrier alert/cron VERIFIED, hub#387 PR-1 docs + PR-2
alert/cron/박제)** → POLICY_FINALIZED (EPIC-evidence-quad-runtime-telemetry 3/3 milestone
COMPLETED, MCT-193 sub-3 LAND 시점). status frontmatter = **Accepted** (Q6=A 구현 LAND =
transition, PR-2 실 alert/cron LAND 후 전환 — false-Accepted 차단).
