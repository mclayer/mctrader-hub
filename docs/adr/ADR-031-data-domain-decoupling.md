...
### §D5 amendment box draft (MCT-187 Phase 1 박제 — VERIFIED 박제는 MCT-187 Phase 2 PR2 시점)

> **§D5 VERIFIED 아님 — Phase 1 outline 박제**. §D5 `data-only-extension-invariant` owner =
> **MCT-187** (본 Story). Phase 2 PR1 (data invariant test LAND) + Phase 2 PR2 (박제) 후
> VERIFIED 확정. 본 box = Phase 1 설계 lane 아웃라인 박제.
...
- **owner Story**: MCT-187 (sequential_phase 6)
- **Phase 0 실증** (2026-05-17, origin/main HEAD 22e2ece):
  - `adapters.py` bithumb/upbit 2-branch 등록 완비 (58 lines, `get_candle_provider` + `get_ws_stream`)
  - `tests/test_adapters.py` 6 TC 기존 존재 (기능 단위 확인)
  - **본 Story = invariant test 신규 박제 + runbook 신규 + §D5 VERIFIED amendment box 확정**
- **VERIFIED 박제 예정 항목** (MCT-187 Phase 2 PR2 시점 확정):
  - `tests/test_multi_exchange_invariant.py` 5 TC PASS (AC-1 carrier)
  - data full suite 회귀 0 (AC-4)
  - `docs/runbooks/add-new-exchange.md` 3-step 절차 LAND (AC-2)
  - D5-row ↔ scope_manifest `§design_decisions.D5` ↔ Change Plan §3 1:1 reconcile (§3.6.1 gate v2)

D6/D7 = 후속 Story owner (MCT-188) — §D5 VERIFIED = MCT-187 Phase 2 PR2 후.
