...
- **JSON schema (v1)**: 11 top-level field + metrics 5 nested field. Top-level: `schema_version` / `node_id` / `collector_run_id` / `version` / `started_at` / `now` / `uptime_seconds` / `ws_state` / `last_event_ts_per_tier` / `queue_depth` / `metrics`. `metrics` 의 nested 5: `events_per_sec` / `dup_skip_count` / `quarantine_count` / `ws_reconnect_count` / `backfill_pending_seconds`
...
mctrader-data repo (`c:/workspace/mclayer/mctrader-data/`) 측 X2 변경 surface. 본 §4 = X2 의 code/test surface **13-entry** (spec §5.2 의 12-entry surface 중 X2 구현 대상 9 + 신규 test 4 의 합). X3 scan-side / X4 coverage / X5-X7 = 별도 child Story 의 scope.
...
| `src/mctrader_data/collector.py` | `--node-id` 받아서 internal collector run 에 propagate. heartbeat writer task 를 async loop 에 attach (5s interval, tier 별 `last_event_ts` hook 제공). `collector_run_id` 생성 시 node_id prefix 포함 — **format = `{node_id}-{UTC_compact_ts}`** (e.g., `NODE_A-20260505T223456Z`, manifest.py 의 기존 hash 방식 변경). heartbeat task shutdown ordering = collector main task cancel 시 heartbeat task 도 cancel + final atomic write 후 정상 종료 (line 186-190 의 `asyncio.gather` cleanup 패턴 확장) | 기존 `async def run()` (line 73 / 175) 위치에 heartbeat task spawn |
...
| `src/mctrader_data/storage.py` | `write_candles()` (line 78) 가 `node_id` arg 받아 path 와 file name (`{collector_run_id}-{batch_seq}.parquet`) 생성. **`batch_seq` semantics**: 0 부터 reset every `collector_run_id` (collector restart 마다 새 collector_run_id 발급되므로 batch_seq 도 0 으로 reset, file name collision 0 보장). parquet metadata 에 `node_id` field 추가 (pyarrow `metadata` 인자) | T1 writer |
...
| `src/mctrader_data/manifest.py` | `run-{collector_run_id}.json` payload 에 `node_id` field 추가 (heartbeat collector_run_id 와 동일 cross-reference). 기존 `CollectorManifest` (line 20) 의 hash-based collector_run_id 생성 (line 35-44) 을 **`{node_id}-{UTC_compact_ts}` format 으로 변경** (heartbeat / manifest / parquet file name 의 cross-reference key) | manifest 위치 / naming 변경 0 |
...
- **B3 (X2 부분)**: heartbeat JSON atomic write (write-temp → fsync → rename) + mtime 갱신 → `test_heartbeat.py` (atomic write 검증 + cross-host read = tmpdir 2개 process simulation). **freshness 판정 / 30s gate / consumer alert** 자체는 X4 (CLI `mctrader-data status` exit code) + X6 (web banner) 책임 — X2 는 producer 측 contract 만 enforce.
...
7. **Heartbeat task shutdown race** (Codex F-7 PUSH-BACK): collector main task cancel 시 heartbeat task 가 마지막 atomic write 중일 수 있음. `collector.py` 의 `asyncio.gather(...)` cleanup (line 186-190) 패턴 확장 — heartbeat task 도 `cancel()` + `await` 후 final flush 보장. **race window**: heartbeat task 의 fsync 진행 중에 cancel 발생 → temp file 만 남고 main file 변경 없음 (atomic rename 의 last-good 유지). consumer 영향 0.
8. **Disk full at temp-write** (Codex F-5 PUSH-BACK): heartbeat write-temp 시 disk full → write 실패. main file 은 last-good 유지, atomic rename 미발생. consumer 입장 mtime 정지 → 30s freshness gate 가 red 신호. **mitigation**: heartbeat writer 가 write-temp 실패 시 exception log + 다음 5s loop 에서 재시도 (collector main task 는 영향 없음, X2 의 writer 가 disk space 자체 관리하지 않음 — host-side prereq).
9. **Heartbeat schema mismatch on read** (Codex F-5 PUSH-BACK): consumer (X4 CLI / X6 web) 가 v2 schema 의 heartbeat 를 v1 으로 read 또는 그 반대 — heartbeat-schema.v1 본문의 "schema_version mismatch 시 warning + best-effort parse" 정책 enforce. X2 는 producer 측 schema_version="heartbeat.v1" 명시 의무, consumer 측 mismatch handling 은 X4/X6 책임.
10. **NFS file lock**: heartbeat writer 는 lock 미사용 — atomic rename 만으로 reader-writer 격리 보장 (POSIX rename(2) atomicity). NFS file lock (lockd / NLM) 의존 0 → 본 design 의 의도된 simplification.
...
- **read-side scan / dedup / quarantine 알고리즘** → X3 (mctrader-data, scan-side merge + dedup module)
- **coverage diagnostic + status CLI** → X4 (mctrader-data, `mctrader-data status` subcommand + heartbeat consumer freshness 판정 + exit code)
...
Phase 2 implementation = `mclayer/mctrader-data` PR #8 (main merged, branch `feat/MCT-91-collector-ha-writer-heartbeat` squash-deleted).

### 8.1 Implementation summary

mctrader-data 0.5.0 → 0.6.0. 9 Task (Task 0 Preflight + Task 1-8 TDD red-green-refactor + Task 9 Release):

| Task | Module / Test | Commit |
|---|---|---|
| 0 | `pyproject.toml` 0.5.0→0.6.0 | chore commit |
| 1 | `heartbeat.py` (NEW, 137 LOC) + `test_heartbeat.py` (8 test) | atomic write/fsync/replace + 5s loop + ws_state Literal + last-good on OSError + schema_version mismatch warning |
| 2 | `cli.py` collect subcommand HA flags (3 신규 click option) + 3 test | --node-id (default hostname) / --heartbeat-interval / --heartbeat-root |
| 3 | `path.py` `derive_partition_path(node_id=None)` + 3 test | leaf 직전 `node={node_id}` Hive level (ADR-009 §D2.1) |
| 4 | `manifest.py` + `lineage.py` `node_id` field + collector_run_id format + legacy hex regression test | `{node_id}-{UTC_compact_ts}` (HA) / sha256[:16] (legacy) 분기 |
| 5 | `storage.py write_candles()` HA kwargs + parquet metadata + 4 test | `{collector_run_id}-{batch_seq}.parquet` naming, `node_id` parquet metadata |
| 6 | `tick_storage.py` + `orderbook_storage.py` HA kwargs + T2/T3 logical key 6/8 column 보존 test | ADR-009 §D10.7 / §D11.8 enforcement |
| 7 | `collector.py` CollectorDaemon node_id/run_id propagation + heartbeat_writer reference + MultiSymbolCollector heartbeat task lifecycle (spawn + cancel + final flush) | Codex F-1/F-5 wiring fix 포함 |
| 8 | `test_active_active_writer.py` (NEW, E2E 2 test) | 두 collector instance write contention 0 + T1 byte-identical full-row + heartbeat 양 file + manifest cross-reference |
| 9 | PR #8 + Codex 7-area review + admin merge | 115 pytest PASS / 0 regression / 0 lint warning |

### 8.2 Caller Propagation (legacy 호환 보장)

모든 신규 인자 default value 의무 (Codex F-4 ADOPT). 영향 받은 caller:

- `cli.py` backfill (line 98 derive_partition_path / 151 write_candles / 164 write_lineage) → 모두 default 사용 (legacy 동작 그대로)
- `paper_storage.py:45` derive_partition_path → default 사용 (paper mode = single-node)
- `paper_lineage.py` (확인 시 호출 0) → 변경 없음
- `orderbook_replay.py:394-398` → reader 측, X3 책임 (forward-only)

### 8.3 Codex 7-area review escalation 4/4 ADOPT

본 PR review (codex:rescue / gpt-5.4): needs-fix → 모두 ADOPT 후 pass:

- F-1 / F-5 (PUSH-BACK, root cause 동일): collector `_handle_event` 가 heartbeat `update_tier_event_ts(tier, ts)` 호출 wiring 추가 (TransactionEvent → "tick" / OrderbookSnapshot/Delta → "orderbook")
- F-4 (NIT): T1 byte-identical 검증 강화 (close column 만 → 전체 row column ts_utc/open/high/low/close/volume/value/schema_version)
- F-7 (NIT): X2-X3 window 운영 caveat PR body 명시 (X3 도착 전 신규 file naming transparency 0 + 3 운영 옵션)

기타 4 area (heartbeat module / API backward compat / manifest run_id format / X3 readiness) = ADOPT-AS-IS.

### 8.4 X2-X3 window 운영 caveat (Codex F-7)

X2 main merge 후 X3 (mctrader-data MCT-X3) 도착 전에는 collector daemon 의 신규 file naming 이 기존 reader (`tier_coverage`, `orderbook_replay`) 에 transparent 하지 않음. 사용자 운영 옵션 3개:

- (a) X3 도착 대기 후 collector daemon 활성화
- (b) collector daemon 비활성화 + `mctrader-data backfill` 만 사용 (legacy file naming 유지)
- (c) 단일 node `--node-id=DEFAULT` 운영 (active-active 효과 0, dedup 우회)
...
### 9.1 Test results (mctrader-data PR #8)

- **115 pytest PASS** (37 신규 X2 test + 78 기존 regression 0):
  - 신규: heartbeat (8) / cli HA (3) / path HA (3) / manifest HA + legacy regression (4) / lineage HA (2) / storage HA (4) / tick_storage HA (4) / orderbook_storage HA (4) / collector wiring + lifecycle (3) / E2E active-active (2)
  - 기존: 모든 0.5.0 test regression 0

- **CI**:
  - Windows: pass (1m7s)
  - Ubuntu: pass (16s, ruff lint clean)
  - check-gate: pass
  - phase-gate-mergeable: ACTION_REQUIRED (codeforge phase label sequencing — admin merge autonomy 적용, feedback memory)

### 9.2 Codex review 결과

- Phase 2 plan review (mctrader-hub PR #95): 6/6 ADOPT (1 REJECT + 4 PUSH-BACK + 2 NIT)
- Phase 2 PR review (mctrader-data PR #8): 4/4 ADOPT (2 PUSH-BACK + 2 NIT)
- Sonnet decider escalation 0건 (모두 자율 ADOPT)

### 9.3 ADR-009 enforcement 검증

- §D2.1 partition `node=` Hive level: `path.py` + writer 3개 + integration test 검증
- §D10.7 T2 tick logical key 6 column 보존: `test_tick_storage.py::test_tick_logical_key_columns_preserved`
- §D11.8 T3 orderbook logical key 8 column 보존: `test_orderbook_storage.py::test_orderbook_logical_key_columns_preserved`
- heartbeat-schema.v1: `test_heartbeat.py::test_schema_v1_top_level_11_field` (11 top-level + 5 nested metrics)

### 9.4 Phase 2 close marker

- mctrader-data 0.6.0 main merged (commit 8b11b7a)
- mctrader-hub Phase 1 PR #94 + Phase 2 plan PR #95 main merged
- 다음 child Story 후보: MCT-X3 (scan-side dedup), MCT-X4 (status CLI), MCT-X5 (ops scripts), MCT-X6 (web panel), MCT-X7 (Calibration)
